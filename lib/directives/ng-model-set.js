    ...
    return {
            ...
            this.show = function(item){
                ...
                $q.all([item.element(), prev ? prev.element(): undefined]).then(function(resolved){
                    $animate.enter(resolved[0], $element, resolved[1]).then(function(){
                        $timeout(function(){
                            item.compileOpts.transcludeControllers.form.instance.on(item.path, 'change', function(value){
                                if(self.autoHide){
                                    if(value){self.showNext(item.path);}
                                    else {self.hideNext(item.path);}
                                }
                            });
                        });
                    });
        ...
        link: function(scope, element, attrs, ctrls){
            var paths = attrs.include.split(' '),
                ...
                form = ctrls[0],
                ngModelSet = ctrls[1];
            ngModelSet.autoHide = scope.$eval(attrs.autoHide);
            if(!paths.length){
                return;
            }
            ...
            if(ngModelSet.autoHide){
                ngModelSet.showFirst();
            }
            else {
                angular.forEach(paths, function(path){
                    ngModelSet.show(path);
                });
            }
        }
    };
