            ...
            switch(name) {
                case 'Feedback':
                  schema.methods.postToCRM = function(){
                   ...
                   DiscountInfo.findOne({feedback: mongoose.Types.ObjectId(feedback._id)}, function(err, discount){
                     feedback.populate('car', function(err, feedback) {
                      var populateBrand = function(f){f(null, feedback);};
                      if(feedback.car) {
                          populateBrand = feedback.car.populate.bind(feedback.car, 'brand');
                      }
                      populateBrand(function(err, feedback) {
                	    var username = settings.GATEWAY_USERNAME,
                	        ...
                	        legacyData = {
                                mk: feedback.car ? ( feedback.car ? (feedback.car.brand.label + ' ' + feedback.car.label) : feedback.car.fullLabel ) : 'unknown',
                        ...
                        post = request.post(url, { form: data }, function(err, response){
                            if(settings.DEBUG){
